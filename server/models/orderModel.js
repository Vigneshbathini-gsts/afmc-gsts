...
async function getOrderDetails(orderNumber) {
  ...
  ROUND(
    CASE
      WHEN COALESCE(scanned_totals.scanned_total, item_scanned_totals.scanned_total) > 0 AND (od.price IS NULL OR od.price <> 0) THEN COALESCE(scanned_totals.scanned_total, item_scanned_totals.scanned_total) / NULLIF(od.quantity, 0)

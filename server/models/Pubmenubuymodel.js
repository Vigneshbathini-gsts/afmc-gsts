...
async function getIngredientStockQuantitiesFromInventory(connection, ingredientCodes) {
...
async function getIngredientReservedQuantitiesFromOrders(connection, ingredientCodes) {
  const normalizedCodes = [...new Set((Array.isArray(ingredientCodes) ? ingredientCodes : [])
    .map((code) => Number(code))
    .filter((code) => Number.isFinite(code) && code > 0))];
...
async function getOrderSummary(orderNumber) {
  ...
  const [itemRows] = await db.execute(
    `
      SELECT
        ...
        XXINV.CATEGORY_ID AS category_id,
        CASE
  ...
  const formattedItems = reconciledItems.map((row) => {
    const isCocktailItem =
      Number(row.category_id) === 10 &&
      [14, 15].includes(Number(row.subcategory));
    const isFreeItem = row.price !== null && row.price !== undefined && Number(row.price) === 0;
...
// (Removed a duplicate commented-out implementation of `updateOrderLineQuantity` that used to live here.)
  ...
  try {
    ...
    if (isCocktailOrMocktail) {
      ...
      const reservedQuantity = Number(
        reservedMap.get(String(itemId)) || 0
      );
      ...
      if (normalizedQuantity > availableQuantity) {
        const error = new Error(
          `Out of stock. Available quantity: ${availableQuantity}`
        );

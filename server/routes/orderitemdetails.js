...
const getOrderItemDetails = async (req, res) => {
  ...
  // ---------------------------------------------------------
  // FIXED ITEM NAME NORMALIZATION
  // Supports:
  //   "VIRGIN MARY,CHIPS NON VEG"
  //   ["VIRGIN MARY", "CHIPS NON VEG"]
  //   ["VIRGIN MARY,CHIPS NON VEG"]
  // ---------------------------------------------------------
  const normalizeItemNames = (value) => {
  ...
  const itemFilterClause = itemNames.length
    ? `AND UPPER(TRIM(xi.item_name)) IN (${buildInClause(itemNames)})`
    : "";
  ...
  // ---------------------------------------------------------
  // CUSTOM COCKTAIL / MOCKTAIL TOTALS
  // ---------------------------------------------------------
      ...
      SELECT
        ...
        ROUND(
          SUM(
            IFNULL(cm.pegs, 0) *
            (
              IFNULL(stock_prices.base_peg_price, 0)
              *
              (
                1 +
                IFNULL(od_price.profit, 0) / 100
              )
              +
  ...
  const effectiveProfitExpression = `
    CASE
      WHEN
        xi.category_id = 10
        AND xi.sub_category IN (
          ${NON_ALCOHOLIC_LIQUOR_SUBCATEGORY_IDS.join(", ")}
        )
      THEN 0
  ...
  const effectiveSubtotalExpression = `
    CASE
      ...
      WHEN
        xi.sub_category IN (14, 15)
        AND IFNULL(st.scanned_subtotal, 0) > 0
      THEN
        st.scanned_subtotal

      WHEN
        xi.sub_category IN (14, 15)
        AND IFNULL(ct.unit_custom_subtotal, 0) > 0
      THEN
        ct.unit_custom_subtotal
        *
        (
  ...
  const baseQuery = `
    SELECT
      ...
      SUM(
        ${effectiveQuantityExpression}
      ) AS quantity,

      IFNULL(
        SUM(
          ${effectiveSubtotalExpression}
        ),
        0
      ) AS subtotal,
  ...
  const totalQuery = `
    SELECT
      ...
      ROUND(
        IFNULL(SUM(food_pr_charges), 0),
        2
      ) AS food_pr_charges,

      ROUND(
        IFNULL(SUM(totalprofit), 0),
        2
      ) AS totalprofit,
  ...
  const baseValues = [
    ...
    // instead of:
    // ?
    // containing "VIRGIN MARY,CHIPS NON VEG"
    ...itemNames,
  ];

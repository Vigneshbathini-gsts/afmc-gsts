...
export default function AddItem() {
  ...
  const addBarcodeRow = useCallback(async (barcodeValue) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-rose-50 to-white relative overflow-hidden">
      ...
      <div className="relative z-10 p-8">
        ...
        <div className="rounded-[32px] border border-white/70 bg-white/85 shadow-2xl backdrop-blur-md p-8">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_220px_auto] gap-6 items-end">
            <div>
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={loading}
                className="px-5 py-3 rounded-2xl bg-[#5b5b5b] text-white font-semibold flex items-center gap-2 shadow hover:shadow-md disabled:opacity-70"
              >
                <FaSearch />
                {loading ? "Adding..." : "Add"}
              </button>
              ...
              <button
                ...
                className="px-5 py-3 rounded-2xl bg-[#d70652] text-white font-semibold flex items-center gap-2 shadow hover:shadow-md"
              >
                <FaCamera />
                Scan
              </button>
          ...
          {successMessage && (
            <div className="mt-6 rounded-2xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700 flex items-center gap-2">
              <FaCheckCircle />
              {successMessage}
            </div>
          )}
                ...
                {rows.length === 0 ? (
                  ...
                  rows.map((row, index) => (
                    <tr key={`${row.barcode}-${index}`} className="border-t border-gray-100">
                      ...
                      <td className="px-4 py-3 max-w-[180px] truncate" title={row.batchName}>
                        {row.batchName || "-"}
                      </td>
                      <td className="px-4 py-3">
                        <button
                          ...
                          className="inline-flex items-center justify-center w-9 h-9 rounded-full border border-red-200 text-red-600 hover:bg-red-50"
                        >
                          <FaTrash />
                        </button>
                      </td>
          ...
          <div className="mt-8 flex flex-wrap items-center justify-between gap-4">
            <button
              ...
              className="px-6 py-3 rounded-full bg-[#5b5b5b] text-white font-semibold"
            >
              Cancel
            </button>
            ...
            <button
              ...
              className="px-8 py-3 rounded-full bg-green-700 text-white font-semibold flex items-center gap-2 disabled:opacity-70"
            >
              <FaCheckCircle />
